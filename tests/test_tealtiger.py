...
All tests use mocked AsyncDakeraClient — TealTiger does NOT need to be installed.
...
def _make_client() -> MagicMock:
    """Return a mock AsyncDakeraClient with async methods pre-configured."""
    client = MagicMock()
    # All five methods used by the integration are async on AsyncDakeraClient.
    client.store_memory = AsyncMock(return_value={"id": "mem-1"})
    client.batch_recall = AsyncMock(return_value=_make_batch_recall_response())
    client.batch_forget = AsyncMock(return_value=BatchForgetResponse(deleted_count=0))
    client.memory_link = AsyncMock(return_value=None)
    client.knowledge_query = AsyncMock(
        return_value=KgQueryResponse(agent_id="ag-1", node_count=0, edge_count=0, edges=[])
    )
    return client
...
def _make_cost_record(
    ...
    record.breakdown = MagicMock(input_cost=0.03, output_cost=0.02)
...
class TestDakeraCostStorageStore:
    async def test_store_calls_store_memory_with_correct_tags(self) -> None:
        client = _make_client()
    ...
    async def test_store_content_is_json(self) -> None:
        client = _make_client()
    ...
    async def test_store_is_awaitable(self) -> None:
        """Verify store() is a coroutine (async def), not a plain sync method."""
        import asyncio
        ...
        coro = storage.store(record)
        assert asyncio.iscoroutine(coro), "store() must return a coroutine (async def)"
        await coro
...
class TestDakeraCostStorageGet:
    async def test_get_returns_none_when_not_found(self) -> None:
        ...
        result = await storage.get("missing-id")

        assert result is None
        client.batch_recall.assert_called_once()

    async def test_get_passes_correct_filter(self) -> None:
        ...
        result = await storage.get("cost-1")
    ...
    async def test_get_deserializes_json_when_tealtiger_absent(self) -> None:
...
class TestDakeraCostStorageGetByRequestId:
    async def test_returns_empty_list_when_none_found(self) -> None:
        ...
        result = await storage.get_by_request_id("req-xyz")

        assert result == []

    async def test_filter_includes_request_id_tag(self) -> None:
        client = _make_client()
        storage = DakeraCostStorage(client)

        await storage.get_by_request_id("req-abc")
...
class TestDakeraCostStorageGetByAgentId:
    async def test_filter_includes_agent_tag(self) -> None:
        client = _make_client()
        storage = DakeraCostStorage(client)

        await storage.get_by_agent_id("my-agent")

        req = client.batch_recall.call_args.args[0]
        assert "agent:my-agent" in (req.filter.tags or [])

    async def test_date_bounds_forwarded(self) -> None:
        client = _make_client()
        storage = DakeraCostStorage(client)

        await storage.get_by_agent_id(
...
class TestDakeraCostStorageGetByDateRange:
    async def test_filter_uses_governance_tags(self) -> None:
        client = _make_client()
        storage = DakeraCostStorage(client)

        await storage.get_by_date_range("2026-01-01T00:00:00Z", "2026-12-31T23:59:59Z")
...
class TestDakeraCostStorageGetSummary:
    async def test_returns_dict_when_tealtiger_absent(self) -> None:
        import dakera.integrations.tealtiger as tt_mod
        ...
        try:
            ...
            client = _make_client()
            client.batch_recall = AsyncMock(
                return_value=_make_batch_recall_response(
                    [_make_memory(payload), _make_memory(payload, "mem-2")]
                )
            )
            storage = DakeraCostStorage(client)

            result = await storage.get_summary("2026-01-01T00:00:00Z", "2026-12-31T23:59:59Z")
    ...
    async def test_agent_id_filter_applied_when_given(self) -> None:
        client = _make_client()
        storage = DakeraCostStorage(client)

        await storage.get_summary("2026-01-01T00:00:00Z", "2026-12-31T23:59:59Z", agent_id="ag-X")

        req = client.batch_recall.call_args.args[0]
        assert "agent:ag-X" in (req.filter.tags or [])

    async def test_period_is_dict(self) -> None:
        import dakera.integrations.tealtiger as tt_mod
    ...
    async def test_total_tokens_is_dict(self) -> None:
        import dakera.integrations.tealtiger as tt_mod
...
class TestDakeraCostStorageDeleteOlderThan:
    async def test_calls_batch_forget_with_created_before(self) -> None:
        client = _make_client()
        client.batch_forget = AsyncMock(return_value=BatchForgetResponse(deleted_count=3))
        storage = DakeraCostStorage(client)

        count = await storage.delete_older_than("2026-01-01T00:00:00Z")
...
class TestDakeraCostStorageClear:
    async def test_calls_batch_forget_with_governance_tags(self) -> None:
        client = _make_client()
        client.batch_forget = AsyncMock(return_value=BatchForgetResponse(deleted_count=10))
        storage = DakeraCostStorage(client)
...
class TestDakeraDecisionStoreStoreReceipt:
    async def test_deny_uses_highest_importance(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-deny-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("DENY"))

        kwargs = client.store_memory.call_args.kwargs
        assert kwargs["importance"] == 0.95

    async def test_require_approval_importance(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-ra-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("REQUIRE_APPROVAL"))

        assert client.store_memory.call_args.kwargs["importance"] == 0.90

    async def test_redact_importance(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-redact-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("REDACT"))

        assert client.store_memory.call_args.kwargs["importance"] == 0.90

    async def test_transform_importance(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-transform-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("TRANSFORM"))

        assert client.store_memory.call_args.kwargs["importance"] == 0.85

    async def test_degrade_importance(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-degrade-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("DEGRADE"))

        assert client.store_memory.call_args.kwargs["importance"] == 0.85

    async def test_allow_importance(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-allow-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("ALLOW"))

        assert client.store_memory.call_args.kwargs["importance"] == 0.80

    async def test_tags_include_decision_and_ids(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-1"})
        ds = DakeraDecisionStore(client)

        await ds.store_receipt("ag-1", _make_decision("DENY", "corr-99", "policy-99"))
    ...
    async def test_returns_memory_id(self) -> None:
        client = _make_client()
        client.store_memory = AsyncMock(return_value={"id": "mem-abc"})
        ds = DakeraDecisionStore(client)

        mem_id = await ds.store_receipt("ag-1", _make_decision())
...
class TestDakeraDecisionStoreLookupReceipt:
    async def test_returns_none_when_not_found(self) -> None:
        ...
        result = await ds.lookup_receipt("ag-1", "corr-missing")

        assert result is None

    async def test_filter_includes_correlation_id(self) -> None:
        payload = json.dumps(
        ...
        result = await ds.lookup_receipt("ag-1", "corr-42")
...
class TestDakeraDecisionStoreIsTerminal:
    async def test_returns_false_when_no_receipt(self) -> None:
        client = _make_client()
        ds = DakeraDecisionStore(client)
    ...
    async def test_filter_includes_correlation_id_tag(self) -> None:
        ...
        await ds.is_terminal("ag-1", "corr-xyz")
...
class TestDakeraDelegationHelperLink:
    async def test_link_delegation_calls_memory_link(self) -> None:
        client = _make_client()
...
class TestDakeraDelegationHelperGetChain:
    ...
    async def test_chain_starts_with_decision_id(self) -> None:
        client = _make_client()
        client.knowledge_query = AsyncMock(
            return_value=self._make_kg_response("root", ["parent", "grandparent"])
        )
        helper = DakeraDelegationHelper(client)

        chain = await helper.get_delegation_chain("ag-1", "root", max_depth=5)

        assert chain[0] == "root"
        assert "parent" in chain
        assert "grandparent" in chain

    async def test_max_depth_clamped_to_5(self) -> None:
        client = _make_client()
        helper = DakeraDelegationHelper(client)

        await helper.get_delegation_chain("ag-1", "root", max_depth=20)
    ...
    async def test_no_edges_returns_single_item_chain(self) -> None:
        ...
        chain = await helper.get_delegation_chain("ag-1", "root-only", max_depth=3)
